...
public class MyTownDatasource_postgresql extends MyTownDatasource_SQL {
	...
	protected void setUpdates() {
		...
		updates.add(new DBUpdate("03.08.2014.5", "Add TownBlocks Table", "CREATE TABLE IF NOT EXISTS " + prefix + "TownBlocks (Id SERIAL NOT NULL, X int NOT NULL, Z int NOT NULL, Dim int NOT NULL, TownId int NOT NULL, PRIMARY KEY(Id), FOREIGN KEY (TownId) REFERENCES " + prefix
				+ "Towns(Id) ON DELETE CASCADE);"));
		updates.add(new DBUpdate("03.15.2014.1", "Add TownPlots Table", "CREATE TABLE IF NOT EXISTS " + prefix + "TownPlots (Id SERIAL NOT NULL, X1 int NOT NULL, Y1 int NOT NULL, Z1 int NOT NULL, X2 int NOT NULL, Y2 int NOT NULL, Z2 int NOT NULL, Dim int NOT NULL, TownId int NOT NULL, Owner varchar(255) DEFAULT NULL, Rank varchar(1) DEFAULT 'R', PRIMARY KEY(Id), FOREIGN KEY (TownId) REFERENCES " + prefix
				+ "Towns(Id) ON DELETE CASCADE, FOREIGN KEY (Owner) REFERENCES " + prefix + "Residents(UUID) ON DELETE SET NULL);"));
		updates.add(new DBUpdate("03.22.2014.1", "Add ResidentsToTowns Table", "CREATE TABLE IF NOT EXISTS " + prefix + "ResidentsToTowns (Id SERIAL NOT NULL, TownId int NOT NULL, Owner varchar(255) NOT NULL, PRIMARY KEY (Id), FOREIGN KEY (TownId) REFERENCES " + prefix
				+ "Towns(Id) ON DELETE CASCADE, FOREIGN KEY (Owner) REFERENCES " + prefix + "Residents(UUID) ON DELETE CASCADE);"));
		updates.add(new DBUpdate("03.22.2014.2", "Add TownsToNations", "CREATE TABLE IF NOT EXISTS " + prefix + "TownsToNations (Id SERIAL NOT NULL, TownId int NOT NULL, NationId int NOT NULL, Rank varchar(1) DEFAULT 'T', PRIMARY KEY (Id), FOREIGN KEY (TownId) REFERENCES "
