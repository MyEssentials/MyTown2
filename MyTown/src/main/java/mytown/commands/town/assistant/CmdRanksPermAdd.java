...
public class CmdRanksPermAdd extends CommandBase{
	...
	{
		if(args.length < 2)
			throw new WrongUsageException(MyTown.instance.local.getLocalization("mytown.cmd.usage.ranks.perm"));
		Town town = MyTown.instance.datasource.getResident(sender.getCommandSenderName()).getSelectedTown();
		if(MyTown.instance.datasource.getRank(args[0], town) == null)
		...
		if(!CommandUtils.permissionList.containsValue(args[1]))
			throw new CommandException(MyTown.instance.local.getLocalization("mytown.cmd.err.ranks.perm.notexist", args[1]));
		// Adding permission if everything is alright
		town.getRank(args[0]).addPermission(args[1]);
