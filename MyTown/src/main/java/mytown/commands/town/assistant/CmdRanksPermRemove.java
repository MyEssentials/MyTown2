...
@Permission(node="mytown.cmd.assistant.ranks.perm.remove")
public class CmdRanksPermRemove extends CommandBase{
	...
	public void process(ICommandSender sender, String[] args) throws Exception
	{
		...
		if(town.getRank(args[0]).removePermission(args[1]))
		{
			...
			ChatUtils.sendLocalizedChat(sender, MyTown.instance.local, "mytown.notification.town.ranks.perm.remove", args[1], args[0]);
		} else {
			throw new CommandException(MyTown.instance.local.getLocalization("mytown.cmd.err.ranks.perm.remove.failed", args[1]));
		}
	}
}
