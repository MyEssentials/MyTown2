...
import mytown.x_interfaces.IPlotSelector;
...
public class Resident implements IPlotSelector {
    ...
    public void sendMap(int dim, int cx, int cz) {
        ...
        for (int z = cz - heightRad; z <= cz + heightRad; z++) {
            ...
            for (int x = cx - widthRad; x <= cx + widthRad; x++) {
                TownBlock b = X_DatasourceProxy.getDatasource().getTownBlock(dim, x, z, true);
    ...
    public void checkLocation(int oldChunkX, int oldChunkZ, int newChunkX, int newChunkZ, int dimension) {
        if (oldChunkX != newChunkX || oldChunkZ != newChunkZ && player != null) {
            ...
            oldTownBlock = X_DatasourceProxy.getDatasource().getTownBlock(lastDim, oldChunkX, oldChunkZ, true);
            newTownBlock = X_DatasourceProxy.getDatasource().getTownBlock(dimension, newChunkX, newChunkZ, true);
        ...
        if (invitationForms.size() != 0) {
            ...
            if (accepted) {
                try {
                    ...
                    X_DatasourceProxy.getDatasource().linkResidentToTown(this, town, town.getRank("Resident"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
    ...
    public boolean selectBlockForPlot(int dim, int x, int y, int z) {
        TownBlock tb = X_DatasourceProxy.getDatasource().getTownBlock(dim, x, z, false);
    ...
    public boolean makePlotFromSelection(String plotName) {
        ...
        for (int i = x1; i <= x2; i++) {
            for (int j = z1; j <= z2; j++) {
                if (i >> 4 != lastX || j >> 4 != lastZ) {
                    lastX = i >> 4;
                    lastZ = j >> 4;
                    if (!X_DatasourceProxy.getDatasource().hasTownBlock(selectionDim, lastX, lastZ, true, selectionTown)) {
        ...
        try {
            X_DatasourceProxy.getDatasource().insertPlot(plot);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
