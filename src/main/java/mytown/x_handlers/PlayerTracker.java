...
public class PlayerTracker {
	...
	public void onPlayerLogin(PlayerLoggedInEvent ev) {
		...
		try {
			Resident res = X_DatasourceProxy.getDatasource().getOrMakeResident(ev.player);
			res.setOnline(true);
	...
	public void onEnterChunk(EntityEvent.EnteringChunk ev) {
		...
		try {
			Resident res = X_DatasourceProxy.getDatasource().getOrMakeResident(pl);
	...
	public void onPlayerBreaksBlock(BlockEvent.BreakEvent ev) {
		...
		if (!X_DatasourceProxy.getDatasource().hasTownBlock(String.format(TownBlock.keyFormat, ev.world.provider.dimensionId, ev.x >> 4, ev.z >> 4)))
			return;
		else {
			Town town = X_DatasourceProxy.getDatasource().getTownBlock(String.format(TownBlock.keyFormat, ev.world.provider.dimensionId, ev.x >> 4, ev.z >> 4)).getTown();
			ITownFlag flag = town.getFlagAtCoords(ev.x, ev.y, ev.z, "breakBlocks");
			if (flag == null)
				return;
			if (flag.getValue() == true)
				return;
			if (X_DatasourceProxy.getDatasource().getResident(ev.getPlayer().getDisplayName()).isPartOfTown(town))
				return;
			ev.setCanceled(true);
		}
	}
	...
	public void onItemUse(PlayerInteractEvent ev) {
		...
		if (ev.action == Action.RIGHT_CLICK_BLOCK && currentStack.getItem().equals(Items.wooden_hoe) && currentStack.getDisplayName().equals(Constants.EDIT_TOOL_NAME)) {
			Resident res = X_DatasourceProxy.getDatasource().getResident(ev.entityPlayer.getDisplayName());
			if (res == null)
				return;
