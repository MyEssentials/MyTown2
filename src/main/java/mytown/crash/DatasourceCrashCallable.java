...
public class DatasourceCrashCallable implements ICrashCallable {
	...
	public String call() throws Exception {
		...
		str += String.format("Stats (Towns: %s, Residents: %s, Nations: %s, Blocks: %s, Ranks: %s, Plots: %s)\n", datasource.getTownsMap().size(), datasource.getRanksMap().size(), datasource.getNationsMap().size(), datasource.getBlocksMap().size(), datasource.getRanksMap().size(), datasource.getPlotsMap().size());
